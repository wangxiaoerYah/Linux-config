...
function main() {
	local clear_packages=("yay" "open-iscsi" "qemu-user-static-binfmt" "vim" "vi" "nano" "wireguard-tools")
	local need_packages=("dosfstools" "parted" "btrfs-progs" "arch-install-scripts" "neovim" "git" "curl" "wget" "htop" "sudo" "openssh" "tmux" "bash-completion")
	for c_pkg in ${clear_packages[@]}; do
		if [ $(
			pacman -Qi ${c_pkg} >/dev/null 2>&1
			echo $?
		) -eq 0 ]; then
			printf ${LOG_DEBUG} "${c_pkg} install. Start clear."
			pacman -Rss ${c_pkg} --noconfirm
		else
			printf ${LOG_DEBUG} "${c_pkg} not install."
		fi
	done

	for n_pkg in ${need_packages[@]}; do
		if [ $(
			pacman -Qi ${n_pkg} >/dev/null 2>&1
			echo $?
		) -eq 0 ]; then
			printf ${LOG_DEBUG} "${n_pkg} is install."
		else
			printf ${LOG_DEBUG} "${n_pkg} not install. Start install"
			pacman -S ${n_pkg} --noconfirm
		fi
	done
